...
route_adress="http://localhost:3000/location/fav/route"
favpoint_adress="http://localhost:3000/location/fav/point"
...
def uid_get():
    headers = {"Content-Type": "application/json", "Session-Token": token_get()}
    res = r.get("http://localhost:3000/users", headers=headers)
    p=res.json()
    p=dict(p[1])
    uid=p['uid']
    return uid
...
def point_get(uid):
    headers = {"Content-Type": "application/json", "session-token": token_get()}
    data = f'{{"uid": {uid}}}'
    res=r.get("http://localhost:3000/location/latest", headers=headers, data=data)
    return 
...
def month_points_get(uid):
    ...
    res = r.get("http://localhost:3000/location/month_points", headers=headers,data=data)
    return res.json()
